...
#ifdef _WIN32
    static const bool hc16 = false;
#else
    static const bool hc16 = true;
#endif
    const uint16_t * xn16 = hc16 ? ggml_cuda_mmb_cache_lookup(ctx, xn) : nullptr;
    const uint16_t * g16  = hc16 ? ggml_cuda_mmb_cache_lookup(ctx, gate) : nullptr;
    if (xn16 && g16) {
